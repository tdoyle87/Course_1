...
#loop to decide if player wants to hit or stay
  while my_total < 21
    ...
    hit_or_stay = gets.chomp.downcase
      #conditional statement to hit or stay
      ...
      else hit_or_stay == "2"|| hit_or_stay == "stay"
        say "#{players_name} you have #{mycards[0]}, #{mycards[1]} for a total of #{my_total}!"
        break 
      end
  end
      #conditional statment to have dealer hit until he busts or has 17 or more
      while dealer_total < 17
        ...
        if dealer_total > 21
          say "Dealer BUSTS!"
          say "Congratulations #{players_name} you win"
        else dealer_total >= 17 && dealer_total <= 21
        say "The dealer has a total of #{dealer_total}"
        end
        my_total = calculate_total(mycards)
        dealer_total = calculate_total(dealercards)
